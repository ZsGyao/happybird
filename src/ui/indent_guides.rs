use gpui::{
    AnyElement, App, Bounds, Context, Element, ElementId, Entity, Hsla, IntoElement, Pixels, Point,
    Render, Window, fill, point, px, size,
};
...
impl IndentGuideColors {
    ...
    pub fn panel(cx: &App) -> Self {
        Self {
            default: cx.theme().colors.blue_light,
            hover: cx.theme().colors.list_hover,
            active: cx.theme().colors.list_active,
        }
    }
}
